    ...
    items: [
      {
        ...
        homeopathy: [
          ...
          { id: 'am_hom_6', name: 'Inflamación' },
          { id: 'am_hom_7', name: 'Glicólisis(m)' },
          { id: 'am_hom_8', name: 'Ciclo Krebs(t)' },
          { id: 'am_hom_9', name: 'Cadena Resp(n)' },
        ],
        bachFlowers: [
          ...
          { id: 'am_bach_6', name: 'Madreselva' },
          { id: 'am_bach_7', name: 'Estrella de Belén' },
          { id: 'am_bach_8', name: 'Centaura' },
          { id: 'am_bach_9', name: 'Carpe' },
        ],
      },
    ],
  },
...
export default function PatientGuide({ patient }: { patient: PatientWithDetails }) {
  ...
  const handleSelectionChange = <K extends keyof (StandardFormItem & RevitalizationFormItem & MetabolicFormItem)>(
    itemId: string,
    field: K,
    value: (StandardFormItem & RevitalizationFormItem & MetabolicFormItem)[K]
  ) => {
    setSelections(prev => {
      const newSelections = { ...prev };
      if (!newSelections[itemId]) newSelections[itemId] = {};
  ...
  const handleDeleteItem = (
    ...
    setGuideData(prev =>
      prev.map(cat => {
        ...
        if (subCategory && newCat.type === 'METABOLIC') {
          const activator = newCat.items[0] as any;
          activator[subCategory] = activator[subCategory].filter(
            (it: MetabolicActivatorItem) => it.id !== itemId
          );
        } else if (newCat.type === 'STANDARD') {
          newCat.items = (newCat.items as StandardGuideItem[]).filter(it => it.id !== itemId);
        }
        return newCat;
      })
    );
  ...
  const renderRevitalizationItem = (item: RevitalizationGuideItem) => (
    <div
      key={item.id}
      className="p-3 bg-blue-50 rounded-md transition-all border border-blue-200"
    >
      <div className="flex items-center flex-wrap gap-x-4 gap-y-2">
        <input
          type="checkbox"
          id={item.id}
          checked={(selections[item.id] as RevitalizationFormItem)?.selected || false}
          onChange={e => handleSelectionChange(item.id, 'selected', e.target.checked)}
          className="w-5 h-5 accent-primary"
        />
        <label htmlFor={item.id} className="flex-grow font-semibold text-blue-800">
          {item.name}
        </label>
      </div>
      {(selections[item.id] as RevitalizationFormItem)?.selected && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3 pl-9">
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700">Complejo B:</label>
            <input
              type="text"
              placeholder="cc"
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700">Bioquel:</label>
            <input
              type="text"
              placeholder="cc"
  ...
  const renderItem = (
    item: StandardGuideItem | MetabolicActivatorItem,
    categoryId: string,
    subCategory?: 'homeopathy' | 'bachFlowers'
  ) => (
    <div
      key={item.id}
      className="p-3 bg-gray-50 rounded-md transition-all hover:bg-gray-100"
    >
      <div className="flex items-center flex-wrap gap-x-4 gap-y-2">
        <input
          type="checkbox"
          id={item.id}
          checked={(selections[item.id] as StandardFormItem)?.selected || false}
          onChange={e => handleSelectionChange(item.id, 'selected', e.target.checked)}
          className="w-5 h-5 accent-primary"
        />
        <label htmlFor={item.id} className="flex-grow font-medium text-gray-800">
          {item.name}
        </label>
        ...
        <button
          type="button"
          onClick={() => handleDeleteItem(categoryId, item.id, subCategory)}
          className="text-gray-400 hover:text-red-500 transition-colors ml-auto"
        >
          <FaTrash />
        </button>
      </div>
      {(selections[item.id] as StandardFormItem)?.selected && !('dose' in item) && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2 pl-9">
          <input
            type="text"
            placeholder="Cant."
            value={(selections[item.id] as StandardFormItem)?.qty || ''}
            onChange={e => handleSelectionChange(item.id, 'qty', e.target.value)}
            className="input text-sm py-1"
          />
          <select
            value={(selections[item.id] as StandardFormItem)?.freq || ''}
          ...
          <input
            ...
            value={(selections[item.id] as StandardFormItem)?.custom || ''}
  ...
  return (
    <div className="space-y-6">
      <div className="bg-primary text-white p-4 rounded-lg flex justify-between items-center flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <input
            ...
            className="bg-white/20 border-none rounded-md p-2 text-sm text-black"
          />
        </div>
      </div>
          ...
          {openCategories[category.id] && (
            <div className="p-4">
              ...
              {category.type === 'REVITALIZATION' && (
                <div className="space-y-4">
                  {(category.items as RevitalizationGuideItem[]).map(item =>
                    renderRevitalizationItem(item)
                  )}
                </div>
              )}
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <div className="space-y-3">
                      {((category.items[0] as any).homeopathy as MetabolicActivatorItem[]).map(item =>
                        renderItem(item, category.id, 'homeopathy')
                      )}
                      <div className="flex items-center gap-2 pt-3 border-t">
                        <input
                          type="text"
                          placeholder="Añadir…"
                          value={newItemInputs[`${category.id}-homeopathy`] || ''}
                          onChange={e =>
                            setNewItemInputs(prev => ({
                              ...prev,
                              [`${category.id}-homeopathy`]: e.target.value,
                            }))
                          }
                          className="input flex-grow text-sm py-1"
                        />
                        <button
                          type="button"
                          onClick={() => handleAddNewItem(category.id, 'homeopathy')}
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {((category.items[0] as any).bachFlowers as MetabolicActivatorItem[]).map(item =>
                        renderItem(item, category.id, 'bachFlowers')
                      )}
                      <div className="flex items-center gap-2 pt-3 border-t">
                        <input
                          type="text"
                          placeholder="Añadir…"
                          value={newItemInputs[`${category.id}-bachFlowers`] || ''}
                          onChange={e =>
                            setNewItemInputs(prev => ({
                              ...prev,
                              [`${category.id}-bachFlowers`]: e.target.value,
                            }))
                          }
                          className="input flex-grow text-sm py-1"
                        />
                        <button
                          type="button"
                          onClick={() => handleAddNewItem(category.id, 'bachFlowers')}
                          className="btn-primary py-2 px-3 text-sm"
                        >
                          <FaPlus />
                        </button>
      ...
      {isPreviewOpen && (
        <PatientGuidePreview
          patient={patient}
          formValues={{ guideDate: new Date().toISOString(), selections }}
          guideData={guideData}

...
const PatientGuidePreview = ({ patient, guideData, formValues, onClose }: Props) => {
  ...
  // Lógica de impresión robusta
  const handlePrint = () => {
    // 1. Añade una clase al body para activar los estilos de impresión en globals.css
    document.body.classList.add('printing');
    
    // 2. Llama a la función de impresión del navegador
    window.print();
    
    // 3. Elimina la clase después de que el diálogo de impresión se cierre
    // Se usa un pequeño timeout para asegurar que el renderizado se complete antes de quitar la clase.
    setTimeout(() => {
  ...
  return (
    <>
      {/* Modal de Vista Previa (este contenedor no se imprimirá gracias a la clase 'no-print') */}
      ...
      {/* Usamos un Portal de React para renderizar este div directamente en el body,
          fuera del flujo del modal, para evitar problemas de layout al imprimir. */}
...
// Componente separado para el contenido (ahora autocontenido)
...
>(({ patient, guideData, formValues }, ref) => {
  const { selections, observaciones } = formValues;

  // La función para obtener el icono ahora vive aquí para que el componente sea independiente.

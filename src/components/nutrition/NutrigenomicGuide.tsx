...
export default function NutrigenomicGuide({ patient }: { patient: PatientWithDetails }) {
    ...
    const loadInitialData = useCallback(async () => {
        ...
        if (result.success && result.data) {
            ...
            // ===== MODIFICACIÓN: Lógica para cargar el plan MÁS RECIENTE del paciente =====
            // Asume que los planes vienen ordenados del servidor (desc), o los ordena aquí.
            const latestPlan = patient.foodPlans?.sort((a, b) => 
                new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
            )[0];
            
            // Si existe un plan guardado, se fusiona con la plantilla por defecto.
            if (latestPlan && latestPlan.items) {
                const patientItems = latestPlan.items;
                // Itera sobre cada tipo de comida para añadir los items del paciente
                (Object.keys(initialTemplate) as MealType[]).forEach(mealType => {
                    const itemsForMeal = patientItems.filter(item => item.mealType === mealType);
                    // Reemplaza la lista de la plantilla con la del paciente
                    initialTemplate[mealType] = itemsForMeal;
                });
            }
            // ==============================================================================
    ...
    const handleSavePlan = async () => {
        ...
        if (result.success) {
            ...
            // Aquí se podría recargar la data del paciente para reflejar el nuevo plan en el historial
        } else {
